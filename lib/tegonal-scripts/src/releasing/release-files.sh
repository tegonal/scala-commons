...
function releaseFiles() {
	...
	preReleaseCheckGit -v "$version" -b "$branch"
	...
	updateVersionCommonSteps -v "$version" --project-dir "$projectsRootDir" --pattern "$additionalPattern"
	...
	if [[ $prepareOnly != true ]]; then
		releaseTagPushAndPrepareNext -v "$version" \
			"$branchParamPattern" "$branch" \
			"$projectsRootDirParamPattern" "$projectsRootDir" \
			"$additionalPatternParamPattern" "$additionalPattern" \
			"$nextVersionParamPattern" "$nextVersion"
	else
